...
real pO2c_9(t,x) mmHg;
real pO2p_9(t,x) mmHg;
real pO2m_9(t,x) mmHg;
real dO2tot9c(t,x);
real funcO2_9(t,x);
real dfuncO2_9(t,x);
real pO2new_9(t,x) mmHg;
real pO2c_10(t,x) mmHg;
real pO2p_10(t,x) mmHg;
real pO2m_10(t,x) mmHg;
real dO2tot10c(t,x);
real funcO2_10(t,x);
real dfuncO2_10(t,x);
real pO2new_10(t,x) mmHg;
...
real pO2_9c(t,x) mmHg;    // partial pressure in rbc
real pO2_9p(t,x) mmHg;    // partial pressure in rbc
real pO2_9m(t,x) mmHg;    // partial pressure in rbc
real pO2_10c(t,x) mmHg;    // partial pressure in rbc
real pO2_10p(t,x) mmHg;    // partial pressure in rbc
real pO2_10m(t,x) mmHg;    // partial pressure in rbc
...
realState pCO2_8(t,x);
 ...
 pO2_9p=pO2p_9;
 pO2_9m=pO2m_9;
realState pCO2_9(t,x);
  when (t=t.min) pCO2_9 = pCO2t0_in;
  event(t>t.min) {pCO2_9= pCO2_Invert_in;} // wait untill pCO2rbc calculated
 pO2_10c=pO2c_10;
 pO2_10p=pO2p_10;
 pO2_10m=pO2m_10;
realState pCO2_10(t,x);
  when (t=t.min) pCO2_10 = pCO2t0_in;
  event(t>t.min) {pCO2_10= pCO2_Invert_in;} // wait untill pCO2rbc calculated
...
real delpCO2_9c(t,x) = pCO2_9-pCO20; 
real delpCO2_9p(t,x) = pCO2_9-pCO20; 
real delpCO2_9m(t,x) = pCO2_9-pCO20; 
real delpCO2_10c(t,x) = pCO2_10-pCO20; 
real delpCO2_10p(t,x) = pCO2_10-pCO20; 
real delpCO2_10m(t,x) = pCO2_10-pCO20; 
...
real O2_9c(t,x) M;             // free O2_9c
     O2_9c = alphaO2Sys*pO2_9c; 
real CO2_9c(t,x) M;            // free CO2_9c
     CO2_9c = alphaCO2Sys*pCO2_9;  
real O2_9p(t,x) M;             // free O2_9p
     O2_9p = alphaO2Sys*pO2_9p; 
real CO2_9p(t,x) M;            // free CO2_9p
     CO2_9p = alphaCO2Sys*pCO2_9;  
real O2_9m(t,x) M;             // free O2_9m
     O2_9m = alphaO2Sys*pO2_9m; 
real CO2_9m(t,x) M;            // free CO2_9m
     CO2_9m = alphaCO2Sys*pCO2_9;  
real O2_10c(t,x) M;             // free O2_10c
     O2_10c = alphaO2Sys*pO2_10c; 
real CO2_10c(t,x) M;            // free CO2_10c
     CO2_10c = alphaCO2Sys*pCO2_10;  
real O2_10p(t,x) M;             // free O2_10p
     O2_10p = alphaO2Sys*pO2_10p; 
real CO2_10p(t,x) M;            // free CO2_10p
     CO2_10p = alphaCO2Sys*pCO2_10;  
real O2_10m(t,x) M;             // free O2_10m
     O2_10m = alphaO2Sys*pO2_10m; 
real CO2_10m(t,x) M;            // free CO2_10m
     CO2_10m = alphaCO2Sys*pCO2_10;  
// p50 adjustments:
...
real P501_9c(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_9c(t,x) = P500 + 1.7*(4.28e-2*delpCO2_9c + 3.64e-5*(1 mmHg^-1)*delpCO2_9c^2); // all standard conditions, except CO2
real P503_9c(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_9c(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_9c(t,x) mmHg;             // Final P50_9c value for 50% O2 binding to Hb
     P50_9c = P500*(P501_9c/P500)*(P502_9c/P500)*(P503_9c/P500)*(P504_9c/P500); 
real C50_9c(t,x) M;
     C50_9c = alphaO2Sys*P50_9c; 
real P501_9p(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_9p(t,x) = P500 + 1.7*(4.28e-2*delpCO2_9p + 3.64e-5*(1 mmHg^-1)*delpCO2_9p^2); // all standard conditions, except CO2
real P503_9p(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_9p(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_9p(t,x) mmHg;             // Final P50_9p value for 50% O2 binding to Hb
     P50_9p = P500*(P501_9p/P500)*(P502_9p/P500)*(P503_9p/P500)*(P504_9p/P500); 
real C50_9p(t,x) M;
     C50_9p = alphaO2Sys*P50_9p; 
real P501_9m(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_9m(t,x) = P500 + 1.7*(4.28e-2*delpCO2_9m + 3.64e-5*(1 mmHg^-1)*delpCO2_9m^2); // all standard conditions, except CO2
real P503_9m(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_9m(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_9m(t,x) mmHg;             // Final P50_9m value for 50% O2 binding to Hb
     P50_9m = P500*(P501_9m/P500)*(P502_9m/P500)*(P503_9m/P500)*(P504_9m/P500); 
real C50_9m(t,x) M;
     C50_9m = alphaO2Sys*P50_9m; 
real P501_10c(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_10c(t,x) = P500 + 1.7*(4.28e-2*delpCO2_10c + 3.64e-5*(1 mmHg^-1)*delpCO2_10c^2); // all standard conditions, except CO2
real P503_10c(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_10c(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_10c(t,x) mmHg;             // Final P50_10c value for 50% O2 binding to Hb
     P50_10c = P500*(P501_10c/P500)*(P502_10c/P500)*(P503_10c/P500)*(P504_10c/P500); 
real C50_10c(t,x) M;
     C50_10c = alphaO2Sys*P50_10c; 
real P501_10p(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_10p(t,x) = P500 + 1.7*(4.28e-2*delpCO2_10p + 3.64e-5*(1 mmHg^-1)*delpCO2_10p^2); // all standard conditions, except CO2
real P503_10p(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_10p(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_10p(t,x) mmHg;             // Final P50_10p value for 50% O2 binding to Hb
     P50_10p = P500*(P501_10p/P500)*(P502_10p/P500)*(P503_10p/P500)*(P504_10p/P500); 
real C50_10p(t,x) M;
     C50_10p = alphaO2Sys*P50_10p; 
real P501_10m(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_10m(t,x) = P500 + 1.7*(4.28e-2*delpCO2_10m + 3.64e-5*(1 mmHg^-1)*delpCO2_10m^2); // all standard conditions, except CO2
real P503_10m(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_10m(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_10m(t,x) mmHg;             // Final P50_10m value for 50% O2 binding to Hb
     P50_10m = P500*(P501_10m/P500)*(P502_10m/P500)*(P503_10m/P500)*(P504_10m/P500); 
real C50_10m(t,x) M;
     C50_10m = alphaO2Sys*P50_10m; 
...
// Hill coefficient; unitless (redefined as a function pO2_9c)
real nH_9c(t,x) dimensionless;
     nH_9c = alpha-beta*10^(-pO2_9c/gamma); // pO2_9c dependent variable nH_9c
// Hill coefficient; unitless (redefined as a function pO2_9p)
real nH_9p(t,x) dimensionless;
     nH_9p = alpha-beta*10^(-pO2_9p/gamma); // pO2_9p dependent variable nH_9p
// Hill coefficient; unitless (redefined as a function pO2_9m)
real nH_9m(t,x) dimensionless;
     nH_9m = alpha-beta*10^(-pO2_9m/gamma); // pO2_9m dependent variable nH_9m
// Hill coefficient; unitless (redefined as a function pO2_10c)
real nH_10c(t,x) dimensionless;
     nH_10c = alpha-beta*10^(-pO2_10c/gamma); // pO2_10c dependent variable nH_10c
// Hill coefficient; unitless (redefined as a function pO2_10p)
real nH_10p(t,x) dimensionless;
     nH_10p = alpha-beta*10^(-pO2_10p/gamma); // pO2_10p dependent variable nH_10p
// Hill coefficient; unitless (redefined as a function pO2_10m)
real nH_10m(t,x) dimensionless;
     nH_10m = alpha-beta*10^(-pO2_10m/gamma); // pO2_10m dependent variable nH_10m
...
// Compute the apparent equilibrium constant of Hb with O2_9c and CO2_9c (KHbO2 and KHbCO2); 
// O2_9c and CO2_9c saturations of Hb (SHbO2 and SHbCO2); and O2_9c and CO2_9c contents in blood. 
real K4p_9c(t,x) 1/M;  
     K4p_9c = (1 M^-1)*((O2_9c*(1 M^-1))^(nH_9c-1)*(K2p*BPH1*CO2_9c+BPH3))/( ((1 M^-1)*C50_9c)^nH_9c*(K3p*BPH2*CO2_9c+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_9p and CO2_9p (KHbO2 and KHbCO2); 
// O2_9p and CO2_9p saturations of Hb (SHbO2 and SHbCO2); and O2_9p and CO2_9p contents in blood. 
real K4p_9p(t,x) 1/M;  
     K4p_9p = (1 M^-1)*((O2_9p*(1 M^-1))^(nH_9p-1)*(K2p*BPH1*CO2_9p+BPH3))/( ((1 M^-1)*C50_9p)^nH_9p*(K3p*BPH2*CO2_9p+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_9m and CO2_9m (KHbO2 and KHbCO2); 
// O2_9m and CO2_9m saturations of Hb (SHbO2 and SHbCO2); and O2_9m and CO2_9m contents in blood. 
real K4p_9m(t,x) 1/M;  
     K4p_9m = (1 M^-1)*((O2_9m*(1 M^-1))^(nH_9m-1)*(K2p*BPH1*CO2_9m+BPH3))/( ((1 M^-1)*C50_9m)^nH_9m*(K3p*BPH2*CO2_9m+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_10c and CO2_10c (KHbO2 and KHbCO2); 
// O2_10c and CO2_10c saturations of Hb (SHbO2 and SHbCO2); and O2_10c and CO2_10c contents in blood. 
real K4p_10c(t,x) 1/M;  
     K4p_10c = (1 M^-1)*((O2_10c*(1 M^-1))^(nH_10c-1)*(K2p*BPH1*CO2_10c+BPH3))/( ((1 M^-1)*C50_10c)^nH_10c*(K3p*BPH2*CO2_10c+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_10p and CO2_10p (KHbO2 and KHbCO2); 
// O2_10p and CO2_10p saturations of Hb (SHbO2 and SHbCO2); and O2_10p and CO2_10p contents in blood. 
real K4p_10p(t,x) 1/M;  
     K4p_10p = (1 M^-1)*((O2_10p*(1 M^-1))^(nH_10p-1)*(K2p*BPH1*CO2_10p+BPH3))/( ((1 M^-1)*C50_10p)^nH_10p*(K3p*BPH2*CO2_10p+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_10m and CO2_10m (KHbO2 and KHbCO2); 
// O2_10m and CO2_10m saturations of Hb (SHbO2 and SHbCO2); and O2_10m and CO2_10m contents in blood. 
real K4p_10m(t,x) 1/M;  
     K4p_10m = (1 M^-1)*((O2_10m*(1 M^-1))^(nH_10m-1)*(K2p*BPH1*CO2_10m+BPH3))/( ((1 M^-1)*C50_10m)^nH_10m*(K3p*BPH2*CO2_10m+BPH4));
...
real KHbO2_9c(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_9c
     KHbO2_9c = K4p_9c*(K3p*BPH2*CO2_9c+BPH4)/(K2p*BPH1*CO2_9c+BPH3); 
real SHbO2_9c(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_9c = KHbO2_9c*O2_9c/(1+KHbO2_9c*O2_9c);
real KHbO2_9p(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_9p
     KHbO2_9p = K4p_9p*(K3p*BPH2*CO2_9p+BPH4)/(K2p*BPH1*CO2_9p+BPH3); 
real SHbO2_9p(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_9p = KHbO2_9p*O2_9p/(1+KHbO2_9p*O2_9p);
real KHbO2_9m(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_9m
     KHbO2_9m = K4p_9m*(K3p*BPH2*CO2_9m+BPH4)/(K2p*BPH1*CO2_9m+BPH3); 
real SHbO2_9m(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_9m = KHbO2_9m*O2_9m/(1+KHbO2_9m*O2_9m);
real KHbO2_10c(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_10c
     KHbO2_10c = K4p_10c*(K3p*BPH2*CO2_10c+BPH4)/(K2p*BPH1*CO2_10c+BPH3); 
real SHbO2_10c(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_10c = KHbO2_10c*O2_10c/(1+KHbO2_10c*O2_10c);
real KHbO2_10p(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_10p
     KHbO2_10p = K4p_10p*(K3p*BPH2*CO2_10p+BPH4)/(K2p*BPH1*CO2_10p+BPH3); 
real SHbO2_10p(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_10p = KHbO2_10p*O2_10p/(1+KHbO2_10p*O2_10p);
real KHbO2_10m(t,x) 1/M;               // Apparent equilibrium constants of Hb with O2_10m
     KHbO2_10m = K4p_10m*(K3p*BPH2*CO2_10m+BPH4)/(K2p*BPH1*CO2_10m+BPH3); 
real SHbO2_10m(t,x) dimensionless;     // Saturation of hemoglobin with oxygen
     SHbO2_10m = KHbO2_10m*O2_10m/(1+KHbO2_10m*O2_10m);
...
real O2free_9c(t,x) = Wrbc*O2_9c; // M (mol O2_9c per L rbc)
real O2bound_9c(t,x) = 4*Hbrbc*SHbO2_9c; // M (mol O2_9c per L rbc)
real O2tot_9c(t,x) = O2free_9c+O2bound_9c;   // M (mol O2_9c per L rbc)
real O2free_9p(t,x) = Wrbc*O2_9p; // M (mol O2_9p per L rbc)
real O2bound_9p(t,x) = 4*Hbrbc*SHbO2_9p; // M (mol O2_9p per L rbc)
real O2tot_9p(t,x) = O2free_9p+O2bound_9p;   // M (mol O2_9p per L rbc)
real O2free_9m(t,x) = Wrbc*O2_9m; // M (mol O2_9m per L rbc)
real O2bound_9m(t,x) = 4*Hbrbc*SHbO2_9m; // M (mol O2_9m per L rbc)
real O2tot_9m(t,x) = O2free_9m+O2bound_9m;   // M (mol O2_9m per L rbc)
real O2free_10c(t,x) = Wrbc*O2_10c; // M (mol O2_10c per L rbc)
real O2bound_10c(t,x) = 4*Hbrbc*SHbO2_10c; // M (mol O2_10c per L rbc)
real O2tot_10c(t,x) = O2free_10c+O2bound_10c;   // M (mol O2_10c per L rbc)
real O2free_10p(t,x) = Wrbc*O2_10p; // M (mol O2_10p per L rbc)
real O2bound_10p(t,x) = 4*Hbrbc*SHbO2_10p; // M (mol O2_10p per L rbc)
real O2tot_10p(t,x) = O2free_10p+O2bound_10p;   // M (mol O2_10p per L rbc)
real O2free_10m(t,x) = Wrbc*O2_10m; // M (mol O2_10m per L rbc)
real O2bound_10m(t,x) = 4*Hbrbc*SHbO2_10m; // M (mol O2_10m per L rbc)
real O2tot_10m(t,x) = O2free_10m+O2bound_10m;   // M (mol O2_10m per L rbc)
...
real pO2old_9(t,x) = pO2new_8; // Initial guess for pO2, otherwise it was previous iteration's value (pO2new_9)
  pO2c_9 = pO2old_9;
  pO2p_9 = pO2old_9 + 1e-2*pO2old_9;
  pO2m_9 = pO2old_9 - 1e-2*pO2old_9;
  dO2tot9c = (O2tot_9p-O2tot_9m)/(pO2p_9-pO2m_9);  // Derivative by central difference
  funcO2_9 = TotO2_Invert_in - O2tot_9c;   // Function to be solved (find pO2 such that funcO2_9 = 0) 
  dfuncO2_9 = -dO2tot9c;        // As TotO2 is an input (constant), dTotO2 = 0
  pO2new_9 = (pO2old_9 - funcO2_9/dfuncO2_9) ;// Newton-Raphson iterative formula
real errO2_9(t,x) = abs(pO2new_9 - pO2old_9)/pO2new_9;   // use this as a check.
real pO2old_10(t,x) = pO2new_9; // Initial guess for pO2, otherwise it was previous iteration's value (pO2new_10)
  pO2c_10 = pO2old_10;
  pO2p_10 = pO2old_10 + 1e-2*pO2old_10;
  pO2m_10 = pO2old_10 - 1e-2*pO2old_10;
  dO2tot10c = (O2tot_10p-O2tot_10m)/(pO2p_10-pO2m_10);  // Derivative by central difference
  funcO2_10 = TotO2_Invert_in - O2tot_10c;   // Function to be solved (find pO2 such that funcO2_10 = 0) 
  dfuncO2_10 = -dO2tot10c;        // As TotO2 is an input (constant), dTotO2 = 0
  pO2new_10 = (pO2old_10 - funcO2_10/dfuncO2_10) ;// Newton-Raphson iterative formula
real errO2_10(t,x) = abs(pO2new_10 - pO2old_10)/pO2new_10;   // use this as a check.
pO2_out = pO2new_10;              // Final Output of inversion
...
real pCO2c_CO2_Invert_7(t,x) mmHg;
real pCO2p_CO2_Invert_7(t,x) mmHg;
real pCO2m_CO2_Invert_7(t,x) mmHg;
real dCO2totCO2_Invert_7c(t,x);
real funcCO2_CO2_Invert_7(t,x);
real dfuncCO2_CO2_Invert_7(t,x);
real pCO2new_CO2_Invert_7(t,x) mmHg;
real pCO2c_CO2_Invert_8(t,x) mmHg;
real pCO2p_CO2_Invert_8(t,x) mmHg;
real pCO2m_CO2_Invert_8(t,x) mmHg;
real dCO2totCO2_Invert_8c(t,x);
real funcCO2_CO2_Invert_8(t,x);
real dfuncCO2_CO2_Invert_8(t,x);
real pCO2new_CO2_Invert_8(t,x) mmHg;
...
real pO2_CO2_Invert_7(t,x) mmHg;    // partial pressure in rbc
real pO2_CO2_Invert_8(t,x) mmHg;    // partial pressure in rbc
...
real pCO2_CO2_Invert_7c(t,x) mmHg;   // partial pressure in rbc
real pCO2_CO2_Invert_7p(t,x) mmHg;   // partial pressure in rbc
real pCO2_CO2_Invert_7m(t,x) mmHg;   // partial pressure in rbc
real pCO2_CO2_Invert_8c(t,x) mmHg;   // partial pressure in rbc
real pCO2_CO2_Invert_8p(t,x) mmHg;   // partial pressure in rbc
real pCO2_CO2_Invert_8m(t,x) mmHg;   // partial pressure in rbc
// Assign inputs to SHbO2CO2 calcs to get pCO2 given TCO2 in rbc
 ...
 pCO2_CO2_Invert_7c=pCO2c_CO2_Invert_7;
 pCO2_CO2_Invert_7p=pCO2p_CO2_Invert_7;
 pCO2_CO2_Invert_7m=pCO2m_CO2_Invert_7;
 pO2_CO2_Invert_7= pO2_CO2_Invert_in;
 pCO2_CO2_Invert_8c=pCO2c_CO2_Invert_8;
 pCO2_CO2_Invert_8p=pCO2p_CO2_Invert_8;
 pCO2_CO2_Invert_8m=pCO2m_CO2_Invert_8;
 pO2_CO2_Invert_8= pO2_CO2_Invert_in;
...
real delpCO2_CO2_Invert_7c(t,x) = pCO2_CO2_Invert_7c-pCO20; 
real delpCO2_CO2_Invert_7p(t,x) = pCO2_CO2_Invert_7p-pCO20; 
real delpCO2_CO2_Invert_7m(t,x) = pCO2_CO2_Invert_7m-pCO20; 
real delpCO2_CO2_Invert_8c(t,x) = pCO2_CO2_Invert_8c-pCO20; 
real delpCO2_CO2_Invert_8p(t,x) = pCO2_CO2_Invert_8p-pCO20; 
real delpCO2_CO2_Invert_8m(t,x) = pCO2_CO2_Invert_8m-pCO20; 
...
real O2_CO2_Invert_7c(t,x) M;             // free O2_CO2_Invert_7c
     O2_CO2_Invert_7c = alphaO2Sys*pO2_CO2_Invert_7; 
real CO2_CO2_Invert_7c(t,x) M;            // free CO2_CO2_Invert_7c
     CO2_CO2_Invert_7c = alphaCO2Sys*pCO2_CO2_Invert_7c;  
real O2_CO2_Invert_7p(t,x) M;             // free O2_CO2_Invert_7p
     O2_CO2_Invert_7p = alphaO2Sys*pO2_CO2_Invert_7; 
real CO2_CO2_Invert_7p(t,x) M;            // free CO2_CO2_Invert_7p
     CO2_CO2_Invert_7p = alphaCO2Sys*pCO2_CO2_Invert_7p;  
real O2_CO2_Invert_7m(t,x) M;             // free O2_CO2_Invert_7m
     O2_CO2_Invert_7m = alphaO2Sys*pO2_CO2_Invert_7; 
real CO2_CO2_Invert_7m(t,x) M;            // free CO2_CO2_Invert_7m
     CO2_CO2_Invert_7m = alphaCO2Sys*pCO2_CO2_Invert_7m;  
real O2_CO2_Invert_8c(t,x) M;             // free O2_CO2_Invert_8c
     O2_CO2_Invert_8c = alphaO2Sys*pO2_CO2_Invert_8; 
real CO2_CO2_Invert_8c(t,x) M;            // free CO2_CO2_Invert_8c
     CO2_CO2_Invert_8c = alphaCO2Sys*pCO2_CO2_Invert_8c;  
real O2_CO2_Invert_8p(t,x) M;             // free O2_CO2_Invert_8p
     O2_CO2_Invert_8p = alphaO2Sys*pO2_CO2_Invert_8; 
real CO2_CO2_Invert_8p(t,x) M;            // free CO2_CO2_Invert_8p
     CO2_CO2_Invert_8p = alphaCO2Sys*pCO2_CO2_Invert_8p;  
real O2_CO2_Invert_8m(t,x) M;             // free O2_CO2_Invert_8m
     O2_CO2_Invert_8m = alphaO2Sys*pO2_CO2_Invert_8; 
real CO2_CO2_Invert_8m(t,x) M;            // free CO2_CO2_Invert_8m
     CO2_CO2_Invert_8m = alphaCO2Sys*pCO2_CO2_Invert_8m;  
...
real P501_CO2_Invert_7c(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_7c(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_7c + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_7c^2); // all standard conditions, except CO2
real P503_CO2_Invert_7c(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_7c(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_7c(t,x) mmHg;             // Final P50_CO2_Invert_7c value for 50% O2 binding to Hb
     P50_CO2_Invert_7c = P500*(P501_CO2_Invert_7c/P500)*(P502_CO2_Invert_7c/P500)*(P503_CO2_Invert_7c/P500)*(P504_CO2_Invert_7c/P500); 
real C50_CO2_Invert_7c(t,x) M;
     C50_CO2_Invert_7c = alphaO2Sys*P50_CO2_Invert_7c; 
real P501_CO2_Invert_7p(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_7p(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_7p + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_7p^2); // all standard conditions, except CO2
real P503_CO2_Invert_7p(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_7p(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_7p(t,x) mmHg;             // Final P50_CO2_Invert_7p value for 50% O2 binding to Hb
     P50_CO2_Invert_7p = P500*(P501_CO2_Invert_7p/P500)*(P502_CO2_Invert_7p/P500)*(P503_CO2_Invert_7p/P500)*(P504_CO2_Invert_7p/P500); 
real C50_CO2_Invert_7p(t,x) M;
     C50_CO2_Invert_7p = alphaO2Sys*P50_CO2_Invert_7p; 
real P501_CO2_Invert_7m(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_7m(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_7m + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_7m^2); // all standard conditions, except CO2
real P503_CO2_Invert_7m(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_7m(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_7m(t,x) mmHg;             // Final P50_CO2_Invert_7m value for 50% O2 binding to Hb
     P50_CO2_Invert_7m = P500*(P501_CO2_Invert_7m/P500)*(P502_CO2_Invert_7m/P500)*(P503_CO2_Invert_7m/P500)*(P504_CO2_Invert_7m/P500); 
real C50_CO2_Invert_7m(t,x) M;
     C50_CO2_Invert_7m = alphaO2Sys*P50_CO2_Invert_7m; 
real P501_CO2_Invert_8c(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_8c(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_8c + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_8c^2); // all standard conditions, except CO2
real P503_CO2_Invert_8c(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_8c(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_8c(t,x) mmHg;             // Final P50_CO2_Invert_8c value for 50% O2 binding to Hb
     P50_CO2_Invert_8c = P500*(P501_CO2_Invert_8c/P500)*(P502_CO2_Invert_8c/P500)*(P503_CO2_Invert_8c/P500)*(P504_CO2_Invert_8c/P500); 
real C50_CO2_Invert_8c(t,x) M;
     C50_CO2_Invert_8c = alphaO2Sys*P50_CO2_Invert_8c; 
real P501_CO2_Invert_8p(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_8p(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_8p + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_8p^2); // all standard conditions, except CO2
real P503_CO2_Invert_8p(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_8p(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_8p(t,x) mmHg;             // Final P50_CO2_Invert_8p value for 50% O2 binding to Hb
     P50_CO2_Invert_8p = P500*(P501_CO2_Invert_8p/P500)*(P502_CO2_Invert_8p/P500)*(P503_CO2_Invert_8p/P500)*(P504_CO2_Invert_8p/P500); 
real C50_CO2_Invert_8p(t,x) M;
     C50_CO2_Invert_8p = alphaO2Sys*P50_CO2_Invert_8p; 
real P501_CO2_Invert_8m(t,x) = P500 + 1.2*(1 mmHg)*(-21.279*delpHrbc + 8.872*delpHrbc^2 - 1.47*delpHrbc^3); // all standard conditions, except pH
real P502_CO2_Invert_8m(t,x) = P500 + 1.7*(4.28e-2*delpCO2_CO2_Invert_8m + 3.64e-5*(1 mmHg^-1)*delpCO2_CO2_Invert_8m^2); // all standard conditions, except CO2
real P503_CO2_Invert_8m(t,x) = P500 + 1.0*(1 mmHg)*(795.633533*(1 M^-1)*delDPGrbc - 19660.8947*(1 M^-2)*delDPGrbc^2); // all standard conditions, except DPG
real P504_CO2_Invert_8m(t,x) = P500 + 0.98*(1 mmHg)*(1.4945*(1 K^-1)*delTemp + 4.335e-2*(1 K^-2)*delTemp^2 + 7e-4*(1 K^-3)*delTemp^3); // all standard conditions, except T
real P50_CO2_Invert_8m(t,x) mmHg;             // Final P50_CO2_Invert_8m value for 50% O2 binding to Hb
     P50_CO2_Invert_8m = P500*(P501_CO2_Invert_8m/P500)*(P502_CO2_Invert_8m/P500)*(P503_CO2_Invert_8m/P500)*(P504_CO2_Invert_8m/P500); 
real C50_CO2_Invert_8m(t,x) M;
     C50_CO2_Invert_8m = alphaO2Sys*P50_CO2_Invert_8m; 
...
// Hill coefficient; unitless (redefined as a function pO2_CO2_Invert_7)
real nH_CO2_Invert_7c(t,x) dimensionless;
     nH_CO2_Invert_7c = alpha-beta*10^(-pO2_CO2_Invert_7/gamma); // pO2_CO2_Invert_7 dependent variable nH_CO2_Invert_7c
real nH_CO2_Invert_7p(t,x) dimensionless;
     nH_CO2_Invert_7p = alpha-beta*10^(-pO2_CO2_Invert_7/gamma); // pO2_CO2_Invert_7 dependent variable nH_CO2_Invert_7p
real nH_CO2_Invert_7m(t,x) dimensionless;
     nH_CO2_Invert_7m = alpha-beta*10^(-pO2_CO2_Invert_7/gamma); // pO2_CO2_Invert_7 dependent variable nH_CO2_Invert_7m
// Hill coefficient; unitless (redefined as a function pO2_CO2_Invert_8)
real nH_CO2_Invert_8c(t,x) dimensionless;
     nH_CO2_Invert_8c = alpha-beta*10^(-pO2_CO2_Invert_8/gamma); // pO2_CO2_Invert_8 dependent variable nH_CO2_Invert_8c
real nH_CO2_Invert_8p(t,x) dimensionless;
     nH_CO2_Invert_8p = alpha-beta*10^(-pO2_CO2_Invert_8/gamma); // pO2_CO2_Invert_8 dependent variable nH_CO2_Invert_8p
real nH_CO2_Invert_8m(t,x) dimensionless;
     nH_CO2_Invert_8m = alpha-beta*10^(-pO2_CO2_Invert_8/gamma); // pO2_CO2_Invert_8 dependent variable nH_CO2_Invert_8m
...
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_7c and CO2_CO2_Invert_7c (KHbO2 and KHbCO2); 
// O2_CO2_Invert_7c and CO2_CO2_Invert_7c saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_7c and CO2_CO2_Invert_7c contents in blood. 
real K4p_CO2_Invert_7c(t,x) 1/M;  
     K4p_CO2_Invert_7c = (1 M^-1)*((O2_CO2_Invert_7c*(1 M^-1))^(nH_CO2_Invert_7c-1)*(K2p*BPH1*CO2_CO2_Invert_7c+BPH3))/( ((1 M^-1)*C50_CO2_Invert_7c)^nH_CO2_Invert_7c*(K3p*BPH2*CO2_CO2_Invert_7c+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_7p and CO2_CO2_Invert_7p (KHbO2 and KHbCO2); 
// O2_CO2_Invert_7p and CO2_CO2_Invert_7p saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_7p and CO2_CO2_Invert_7p contents in blood. 
real K4p_CO2_Invert_7p(t,x) 1/M;  
     K4p_CO2_Invert_7p = (1 M^-1)*((O2_CO2_Invert_7p*(1 M^-1))^(nH_CO2_Invert_7p-1)*(K2p*BPH1*CO2_CO2_Invert_7p+BPH3))/( ((1 M^-1)*C50_CO2_Invert_7p)^nH_CO2_Invert_7p*(K3p*BPH2*CO2_CO2_Invert_7p+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_7m and CO2_CO2_Invert_7m (KHbO2 and KHbCO2); 
// O2_CO2_Invert_7m and CO2_CO2_Invert_7m saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_7m and CO2_CO2_Invert_7m contents in blood. 
real K4p_CO2_Invert_7m(t,x) 1/M;  
     K4p_CO2_Invert_7m = (1 M^-1)*((O2_CO2_Invert_7m*(1 M^-1))^(nH_CO2_Invert_7m-1)*(K2p*BPH1*CO2_CO2_Invert_7m+BPH3))/( ((1 M^-1)*C50_CO2_Invert_7m)^nH_CO2_Invert_7m*(K3p*BPH2*CO2_CO2_Invert_7m+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_8c and CO2_CO2_Invert_8c (KHbO2 and KHbCO2); 
// O2_CO2_Invert_8c and CO2_CO2_Invert_8c saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_8c and CO2_CO2_Invert_8c contents in blood. 
real K4p_CO2_Invert_8c(t,x) 1/M;  
     K4p_CO2_Invert_8c = (1 M^-1)*((O2_CO2_Invert_8c*(1 M^-1))^(nH_CO2_Invert_8c-1)*(K2p*BPH1*CO2_CO2_Invert_8c+BPH3))/( ((1 M^-1)*C50_CO2_Invert_8c)^nH_CO2_Invert_8c*(K3p*BPH2*CO2_CO2_Invert_8c+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_8p and CO2_CO2_Invert_8p (KHbO2 and KHbCO2); 
// O2_CO2_Invert_8p and CO2_CO2_Invert_8p saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_8p and CO2_CO2_Invert_8p contents in blood. 
real K4p_CO2_Invert_8p(t,x) 1/M;  
     K4p_CO2_Invert_8p = (1 M^-1)*((O2_CO2_Invert_8p*(1 M^-1))^(nH_CO2_Invert_8p-1)*(K2p*BPH1*CO2_CO2_Invert_8p+BPH3))/( ((1 M^-1)*C50_CO2_Invert_8p)^nH_CO2_Invert_8p*(K3p*BPH2*CO2_CO2_Invert_8p+BPH4));
// Compute the apparent equilibrium constant of Hb with O2_CO2_Invert_8m and CO2_CO2_Invert_8m (KHbO2 and KHbCO2); 
// O2_CO2_Invert_8m and CO2_CO2_Invert_8m saturations of Hb (SHbO2 and SHbCO2); and O2_CO2_Invert_8m and CO2_CO2_Invert_8m contents in blood. 
real K4p_CO2_Invert_8m(t,x) 1/M;  
     K4p_CO2_Invert_8m = (1 M^-1)*((O2_CO2_Invert_8m*(1 M^-1))^(nH_CO2_Invert_8m-1)*(K2p*BPH1*CO2_CO2_Invert_8m+BPH3))/( ((1 M^-1)*C50_CO2_Invert_8m)^nH_CO2_Invert_8m*(K3p*BPH2*CO2_CO2_Invert_8m+BPH4));
...
real KHbCO2_CO2_Invert_7c(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_7c
     KHbCO2_CO2_Invert_7c = (K2p*BPH1+K3p*K4p_CO2_Invert_7c*BPH2*O2_CO2_Invert_7c)/(BPH3+K4p_CO2_Invert_7c*BPH4*O2_CO2_Invert_7c);
real SHbCO2_CO2_Invert_7c(t,x) dimensionless;
     SHbCO2_CO2_Invert_7c = KHbCO2_CO2_Invert_7c*CO2_CO2_Invert_7c/(1+KHbCO2_CO2_Invert_7c*CO2_CO2_Invert_7c);
real KHbCO2_CO2_Invert_7p(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_7p
     KHbCO2_CO2_Invert_7p = (K2p*BPH1+K3p*K4p_CO2_Invert_7p*BPH2*O2_CO2_Invert_7p)/(BPH3+K4p_CO2_Invert_7p*BPH4*O2_CO2_Invert_7p);
real SHbCO2_CO2_Invert_7p(t,x) dimensionless;
     SHbCO2_CO2_Invert_7p = KHbCO2_CO2_Invert_7p*CO2_CO2_Invert_7p/(1+KHbCO2_CO2_Invert_7p*CO2_CO2_Invert_7p);
real KHbCO2_CO2_Invert_7m(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_7m
     KHbCO2_CO2_Invert_7m = (K2p*BPH1+K3p*K4p_CO2_Invert_7m*BPH2*O2_CO2_Invert_7m)/(BPH3+K4p_CO2_Invert_7m*BPH4*O2_CO2_Invert_7m);
real SHbCO2_CO2_Invert_7m(t,x) dimensionless;
     SHbCO2_CO2_Invert_7m = KHbCO2_CO2_Invert_7m*CO2_CO2_Invert_7m/(1+KHbCO2_CO2_Invert_7m*CO2_CO2_Invert_7m);
real KHbCO2_CO2_Invert_8c(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_8c
     KHbCO2_CO2_Invert_8c = (K2p*BPH1+K3p*K4p_CO2_Invert_8c*BPH2*O2_CO2_Invert_8c)/(BPH3+K4p_CO2_Invert_8c*BPH4*O2_CO2_Invert_8c);
real SHbCO2_CO2_Invert_8c(t,x) dimensionless;
     SHbCO2_CO2_Invert_8c = KHbCO2_CO2_Invert_8c*CO2_CO2_Invert_8c/(1+KHbCO2_CO2_Invert_8c*CO2_CO2_Invert_8c);
real KHbCO2_CO2_Invert_8p(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_8p
     KHbCO2_CO2_Invert_8p = (K2p*BPH1+K3p*K4p_CO2_Invert_8p*BPH2*O2_CO2_Invert_8p)/(BPH3+K4p_CO2_Invert_8p*BPH4*O2_CO2_Invert_8p);
real SHbCO2_CO2_Invert_8p(t,x) dimensionless;
     SHbCO2_CO2_Invert_8p = KHbCO2_CO2_Invert_8p*CO2_CO2_Invert_8p/(1+KHbCO2_CO2_Invert_8p*CO2_CO2_Invert_8p);
real KHbCO2_CO2_Invert_8m(t,x) 1/M;              // Apparent equilibrium constants of Hb with CO2_CO2_Invert_8m
     KHbCO2_CO2_Invert_8m = (K2p*BPH1+K3p*K4p_CO2_Invert_8m*BPH2*O2_CO2_Invert_8m)/(BPH3+K4p_CO2_Invert_8m*BPH4*O2_CO2_Invert_8m);
real SHbCO2_CO2_Invert_8m(t,x) dimensionless;
     SHbCO2_CO2_Invert_8m = KHbCO2_CO2_Invert_8m*CO2_CO2_Invert_8m/(1+KHbCO2_CO2_Invert_8m*CO2_CO2_Invert_8m);
...
real CO2free_CO2_Invert_7c(t,x) = Wrbc*CO2_CO2_Invert_7c;  // M (mol CO2_CO2_Invert_7c per L rbc)
real CO2bicarb_CO2_Invert_7c(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_7c/Hrbc); // M (mol CO2_CO2_Invert_7c per L rbc)
real CO2bound_CO2_Invert_7c(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_7c; // M (mol CO2_CO2_Invert_7c per L rbc)
real CO2tot1_CO2_Invert_7c(t,x) = CO2free_CO2_Invert_7c+CO2bound_CO2_Invert_7c; // M (mol CO2_CO2_Invert_7c per L rbc)
real CO2tot2_CO2_Invert_7c(t,x) = CO2free_CO2_Invert_7c+CO2bicarb_CO2_Invert_7c+CO2bound_CO2_Invert_7c; // M (mol CO2_CO2_Invert_7c per L rbc)
real CO2free_CO2_Invert_7p(t,x) = Wrbc*CO2_CO2_Invert_7p;  // M (mol CO2_CO2_Invert_7p per L rbc)
real CO2bicarb_CO2_Invert_7p(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_7p/Hrbc); // M (mol CO2_CO2_Invert_7p per L rbc)
real CO2bound_CO2_Invert_7p(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_7p; // M (mol CO2_CO2_Invert_7p per L rbc)
real CO2tot1_CO2_Invert_7p(t,x) = CO2free_CO2_Invert_7p+CO2bound_CO2_Invert_7p; // M (mol CO2_CO2_Invert_7p per L rbc)
real CO2tot2_CO2_Invert_7p(t,x) = CO2free_CO2_Invert_7p+CO2bicarb_CO2_Invert_7p+CO2bound_CO2_Invert_7p; // M (mol CO2_CO2_Invert_7p per L rbc)
real CO2free_CO2_Invert_7m(t,x) = Wrbc*CO2_CO2_Invert_7m;  // M (mol CO2_CO2_Invert_7m per L rbc)
real CO2bicarb_CO2_Invert_7m(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_7m/Hrbc); // M (mol CO2_CO2_Invert_7m per L rbc)
real CO2bound_CO2_Invert_7m(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_7m; // M (mol CO2_CO2_Invert_7m per L rbc)
real CO2tot1_CO2_Invert_7m(t,x) = CO2free_CO2_Invert_7m+CO2bound_CO2_Invert_7m; // M (mol CO2_CO2_Invert_7m per L rbc)
real CO2tot2_CO2_Invert_7m(t,x) = CO2free_CO2_Invert_7m+CO2bicarb_CO2_Invert_7m+CO2bound_CO2_Invert_7m; // M (mol CO2_CO2_Invert_7m per L rbc)
real CO2free_CO2_Invert_8c(t,x) = Wrbc*CO2_CO2_Invert_8c;  // M (mol CO2_CO2_Invert_8c per L rbc)
real CO2bicarb_CO2_Invert_8c(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_8c/Hrbc); // M (mol CO2_CO2_Invert_8c per L rbc)
real CO2bound_CO2_Invert_8c(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_8c; // M (mol CO2_CO2_Invert_8c per L rbc)
real CO2tot1_CO2_Invert_8c(t,x) = CO2free_CO2_Invert_8c+CO2bound_CO2_Invert_8c; // M (mol CO2_CO2_Invert_8c per L rbc)
real CO2tot2_CO2_Invert_8c(t,x) = CO2free_CO2_Invert_8c+CO2bicarb_CO2_Invert_8c+CO2bound_CO2_Invert_8c; // M (mol CO2_CO2_Invert_8c per L rbc)
real CO2free_CO2_Invert_8p(t,x) = Wrbc*CO2_CO2_Invert_8p;  // M (mol CO2_CO2_Invert_8p per L rbc)
real CO2bicarb_CO2_Invert_8p(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_8p/Hrbc); // M (mol CO2_CO2_Invert_8p per L rbc)
real CO2bound_CO2_Invert_8p(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_8p; // M (mol CO2_CO2_Invert_8p per L rbc)
real CO2tot1_CO2_Invert_8p(t,x) = CO2free_CO2_Invert_8p+CO2bound_CO2_Invert_8p; // M (mol CO2_CO2_Invert_8p per L rbc)
real CO2tot2_CO2_Invert_8p(t,x) = CO2free_CO2_Invert_8p+CO2bicarb_CO2_Invert_8p+CO2bound_CO2_Invert_8p; // M (mol CO2_CO2_Invert_8p per L rbc)
real CO2free_CO2_Invert_8m(t,x) = Wrbc*CO2_CO2_Invert_8m;  // M (mol CO2_CO2_Invert_8m per L rbc)
real CO2bicarb_CO2_Invert_8m(t,x) = (Wrbc*Rrbc)*(K1*CO2_CO2_Invert_8m/Hrbc); // M (mol CO2_CO2_Invert_8m per L rbc)
real CO2bound_CO2_Invert_8m(t,x) = 4*Hbrbc*SHbCO2_CO2_Invert_8m; // M (mol CO2_CO2_Invert_8m per L rbc)
real CO2tot1_CO2_Invert_8m(t,x) = CO2free_CO2_Invert_8m+CO2bound_CO2_Invert_8m; // M (mol CO2_CO2_Invert_8m per L rbc)
real CO2tot2_CO2_Invert_8m(t,x) = CO2free_CO2_Invert_8m+CO2bicarb_CO2_Invert_8m+CO2bound_CO2_Invert_8m; // M (mol CO2_CO2_Invert_8m per L rbc)
...
real pCO2old_CO2_Invert_7(t,x) = pCO2new_CO2_Invert_6; // Initial guess for pCO2, otherwise it was previous iteration's value (pCO2new_CO2_Invert_7)
  pCO2c_CO2_Invert_7 = pCO2old_CO2_Invert_7;
  pCO2p_CO2_Invert_7 = pCO2old_CO2_Invert_7 + 1e-2*pCO2old_CO2_Invert_7;
  pCO2m_CO2_Invert_7 = pCO2old_CO2_Invert_7 - 1e-2*pCO2old_CO2_Invert_7;
  dCO2totCO2_Invert_7c = (CO2tot1_CO2_Invert_7p-CO2tot1_CO2_Invert_7m)/(pCO2p_CO2_Invert_7-pCO2m_CO2_Invert_7);  // Derivative by central difference
  funcCO2_CO2_Invert_7 = TotCO2_Invert_in - CO2tot1_CO2_Invert_7c;   // Function to be solved (find pCO2 such that funcO2 = 0) 
  dfuncCO2_CO2_Invert_7 = -dCO2totCO2_Invert_7c;        // As TotCO2 is an input (constant), dTotCO2 = 0
  pCO2new_CO2_Invert_7 = (pCO2old_CO2_Invert_7 - funcCO2_CO2_Invert_7/dfuncCO2_CO2_Invert_7) ;// Newton-Raphson iterative formula
real errCO2_CO2_Invert_7(t,x) = abs(pCO2new_CO2_Invert_7 - pCO2old_CO2_Invert_7)/pCO2new_CO2_Invert_7;   // use this as a check.
real pCO2old_CO2_Invert_8(t,x) = pCO2new_CO2_Invert_7; // Initial guess for pCO2, otherwise it was previous iteration's value (pCO2new_CO2_Invert_8)
  pCO2c_CO2_Invert_8 = pCO2old_CO2_Invert_8;
  pCO2p_CO2_Invert_8 = pCO2old_CO2_Invert_8 + 1e-2*pCO2old_CO2_Invert_8;
  pCO2m_CO2_Invert_8 = pCO2old_CO2_Invert_8 - 1e-2*pCO2old_CO2_Invert_8;
  dCO2totCO2_Invert_8c = (CO2tot1_CO2_Invert_8p-CO2tot1_CO2_Invert_8m)/(pCO2p_CO2_Invert_8-pCO2m_CO2_Invert_8);  // Derivative by central difference
  funcCO2_CO2_Invert_8 = TotCO2_Invert_in - CO2tot1_CO2_Invert_8c;   // Function to be solved (find pCO2 such that funcO2 = 0) 
  dfuncCO2_CO2_Invert_8 = -dCO2totCO2_Invert_8c;        // As TotCO2 is an input (constant), dTotCO2 = 0
  pCO2new_CO2_Invert_8 = (pCO2old_CO2_Invert_8 - funcCO2_CO2_Invert_8/dfuncCO2_CO2_Invert_8) ;// Newton-Raphson iterative formula
real errCO2_CO2_Invert_8(t,x) = abs(pCO2new_CO2_Invert_8 - pCO2old_CO2_Invert_8)/pCO2new_CO2_Invert_8;   // use this as a check.
pCO2_out = pCO2new_CO2_Invert_8;              // Final Output of inversion
FCO2_out = alphaCO2Sys*pCO2_out; // 
SHbCO2_out = SHbCO2_CO2_Invert_8c;             // Final SHbCO2_CO2_Invert_8c from inversion
// -----------------------------------
...
real O2solSys_pc(t,x)	mL/mL/atm, // ml O2 per ml plasma per atm
     CO2solSys_pc(t,x)	mL/mL/atm;  
real TempC_Sys_pc(t,x) celsius;
     TempC_Sys_pc = (TempSys_pc-273.15)* (1 celsius)/(1 K);  // convert kelvin to celsius 
     O2solSys_pc = alphaO20calc*exp(-O2k1*TempC_Sys_pc)+alphaO201calc*exp(-O2k2*TempC_Sys_pc);  
     CO2solSys_pc = alphaCO20calc*exp(-CO2k1*TempC_Sys_pc)+alphaCO201calc*exp(-CO2k2*TempC_Sys_pc);
     alphaO2Sys_pc =  (1 atm)*O2solSys_pc/Rconst/TempSys_pc;
     alphaCO2Sys_pc = (1 atm)* CO2solSys_pc/Rconst/TempSys_pc;
...
// This MML file generated from O2CO2_BTEX_MPC.mpc using MPC v1.01.
